...
class AgentActor(CallableActor):
    ...
    async def run(self, input: ActorRunInput) -> AsyncIterator[ActorRunOutput]:
        logger.info(
            f"AgentActor {self.actor_id} run started for request {input.request_id}, kind: {input.envelope_kind}"
        )
        ...
        if input.envelope_kind == EnvelopeKind.A2A_MESSAGE:
            if not isinstance(input.payload, dict):
                logger.error(
                    f"AgentActor {self.actor_id}: A2A_MESSAGE payload not dict for request {input.request_id}."
                )
                yield {
                    "error": "A2A_MESSAGE payload must be a dictionary",
            ...
            try:
                if self._cancel_requested:  # Checkpoint
                    logger.info(
                        f"AgentActor {self.actor_id} cancelled before A2A_MESSAGE processing for {input.request_id}."
                    )
                    yield {"status": TaskState.CANCELED.value, "detail": "Cancelled before A2A_MESSAGE processing."}
                    return

                a2a_message = A2AMessage.model_validate(input.payload)
                logger.info(
                    f"AgentActor {self.actor_id}: Parsed A2A_MESSAGE for request {input.request_id}, role: {a2a_message.role.value}"
                )
                ...
                if self._cancel_requested:
                    logger.info(
                        f"AgentActor {self.actor_id} cancelled during A2A_MESSAGE processing for {input.request_id}."
                    )
                    yield {"status": TaskState.CANCELED.value, "detail": "Cancelled during A2A_MESSAGE processing."}
                    return
                ...
                logger.error(
                    f"AgentActor {self.actor_id}: A2A_MESSAGE validation failed for request {input.request_id}: {e}",
                    exc_info=True,
                )
                yield {
                    "error": "A2A_MESSAGE payload validation failed",
                    "details": e.errors(),
                    "request_id": input.request_id,
                    "status": TaskState.FAILED.value,
                }
            ...
            logger.info(
                f"AgentActor {self.actor_id} received A2A_TASK. Raw payload: {str(input.payload)[:200]} for request {input.request_id}"
            )
            if self._cancel_requested:
                logger.info(f"AgentActor {self.actor_id} cancelled before A2A_TASK processing for {input.request_id}.")
                yield {"status": TaskState.CANCELED.value, "detail": "Processing cancelled by request."}
                return

            if not isinstance(input.payload, dict):
                logger.error(
                    f"AgentActor {self.actor_id}: A2A_TASK payload is not a dict for request {input.request_id}."
                )
                yield {
                    "error": "A2A_TASK payload must be a dictionary",
                    "request_id": input.request_id,
                    "status": TaskState.FAILED.value,
                }
                return
            try:
                ...
                logger.info(
                    f"AgentActor {self.actor_id}: Successfully parsed A2A_TASK '{a2a_task.task_name or 'unnamed'}' for request {input.request_id}"
                )
                ...
                if self._cancel_requested:
                    logger.info(
                        f"AgentActor {self.actor_id} cancelled during A2A_TASK processing for {input.request_id}."
                    )
                    yield {
                        "status": TaskState.CANCELED.value,
                        "detail": f"A2A_TASK '{a2a_task.task_name or 'unnamed'}' cancelled.",
                    }
                    return
                ...
                while self._paused and not self._cancel_requested:
                    logger.debug(
                        f"AgentActor {self.actor_id} A2A_TASK processing paused for request {input.request_id}. Checking in 1s."
                    )
                    await asyncio.sleep(1)
                if self._cancel_requested:
                    logger.info(
                        f"AgentActor {self.actor_id} A2A_TASK for request {input.request_id} cancelled after pause."
                    )
                    yield {"status": TaskState.CANCELED.value, "detail": "Cancelled after pause during A2A_TASK."}
                    return
                ...
                logger.error(
                    f"AgentActor {self.actor_id}: A2A_TASK payload validation failed for request {input.request_id}: {e}",
                    exc_info=True,
                )
                yield {
                    "error": "A2A_TASK payload validation failed.",
                    "details": e.errors(),
                    "request_id": input.request_id,
                    "status": TaskState.FAILED.value,
                }
            ...
            logger.info(
                f"AgentActor {self.actor_id}: Received CONTROL via run method for request {input.request_id}, payload: {control_data}"
            )
            yield {
                "status": f"AgentActor {self.actor_id} acknowledged control signal in run (should be rare)",
                "request_id": input.request_id,
                "control_info": control_data,
            }
        else:
            logger.warning(
                f"AgentActor {self.actor_id}: Unhandled kind '{input.envelope_kind}' for request {input.request_id}"
            )
            yield {
                "error": f"AgentActor {self.actor_id} cannot handle kind '{input.envelope_kind}'",
                "request_id": input.request_id,
                "status": TaskState.FAILED.value,
            }
...
class PlainActor(CallableActor):
    ...
    def __init__(self, actor_id: str, payload_schema: type[BaseModel], **kwargs: Any):
        ...
        self.capability = Capability(
            accepts={EnvelopeKind.PLAIN_TASK, EnvelopeKind.CONTROL}, payload_schema=payload_schema
        )
        self._cancel_requested: bool = False
        self._paused: bool = False
        if kwargs:
    ...
    async def run(self, input: ActorRunInput) -> AsyncIterator[ActorRunOutput]:
        logger.info(
            f"PlainActor {self.actor_id} received input for request {input.request_id}, kind: {input.envelope_kind}"
        )
        ...
        if input.envelope_kind == EnvelopeKind.PLAIN_TASK:
            if not isinstance(input.payload, dict) or "args" not in input.payload:
                logger.error(
                    f"PlainActor {self.actor_id}: PLAIN_TASK payload for request {input.request_id} is not dict with 'args'. Payload: {str(input.payload)[:200]}"
                )
                yield {
                    "error": f"PlainActor {self.actor_id} expects PLAIN_TASK payload to be a dict with 'args' key.",
                    "request_id": input.request_id,
                    "status": TaskState.FAILED.value,
                }
                return
            ...
            try:
                if self._cancel_requested:
                    logger.info(
                        f"PlainActor {self.actor_id} run for request {input.request_id} cancelled before validation."
                    )
                    yield {"status": TaskState.CANCELED.value, "detail": "Processing cancelled by request."}
                    return
                ...
                logger.info(
                    f"PlainActor {self.actor_id}: Successfully parsed PLAIN_TASK args for request {input.request_id}."
                )
                ...
                for i in range(3):
                    if self._cancel_requested:
                        logger.info(
                            f"PlainActor {self.actor_id} run for request {input.request_id} cancelled during processing step {i + 1}."
                        )
                        yield {
                            "status": TaskState.CANCELED.value,
                            "detail": f"Processing cancelled by request during step {i + 1}.",
                        }
                        return

                    while self._paused and not self._cancel_requested:  # Inner pause check for longer steps
                        logger.debug(
                            f"PlainActor {self.actor_id} processing step {i + 1} paused for request {input.request_id}. Checking in 1s."
                        )
                        await asyncio.sleep(1)
                    if self._cancel_requested:
                        logger.info(
                            f"PlainActor {self.actor_id} run for request {input.request_id} cancelled after pause in step {i + 1}."
                        )
                        yield {"status": TaskState.CANCELED.value, "detail": f"Cancelled after pause in step {i + 1}."}
                        return
                ...
                logger.error(
                    f"PlainActor {self.actor_id}: PLAIN_TASK validation failed for request {input.request_id}: {e}",
                    exc_info=True,
                )
                yield {
                    "error": f"PlainActor {self.actor_id} PLAIN_TASK payload validation failed",
                    "details": e.errors(),
                    "request_id": input.request_id,
                    "status": TaskState.FAILED.value,
                }
                ...
                logger.error(
                    f"PlainActor {self.actor_id}: Unexpected error processing PLAIN_TASK for request {input.request_id}: {e}",
                    exc_info=True,
                )
                yield {
                    "error": f"PlainActor {self.actor_id} encountered an unexpected processing error",
                    "details": str(e),
                    "request_id": input.request_id,
                    "status": TaskState.FAILED.value,
                }
            ...
            control_data = input.payload
            logger.info(
                f"PlainActor {self.actor_id}: Received CONTROL via run method for request {input.request_id}, payload: {control_data}"
            )
            yield {
                "status": f"PlainActor {self.actor_id} acknowledged control signal in run (should be rare)",
                "request_id": input.request_id,
                "control_info": control_data,
            }
            ...
            logger.warning(
                f"PlainActor {self.actor_id}: Unhandled kind '{input.envelope_kind}' for request {input.request_id}"
            )
            yield {
                "error": f"PlainActor {self.actor_id} cannot handle kind '{input.envelope_kind}'",
                "request_id": input.request_id,
                "status": TaskState.FAILED.value,
            }
