...
class LocalDispatcher(Dispatcher):
    ...
    async def submit(self, envelope_dict: dict[_t.Any, _t.Any]) -> str:
        ...
        logger.info(
            f"Processing Envelope request_id: {request_id}, task_id: {current_task_id}, kind: {envelope.kind}, runner_id: {runner_id}"
        )
        ...
        result = DispatchResult(
            runner_id=runner_id, request_id=request_id, task_id=current_task_id, status=TaskState.SUBMITTED.value
        )
    ...
    async def _handle_request(
        ...
        envelope: Envelope,
        result: DispatchResult,
        stream_queue: asyncio.Queue[ActorRunOutput | None],
    ) -> None:
        task_id_for_registry = result.task_id
        logger.info(
            f"Handling task: {task_id_for_registry}, envelope_id: {envelope.request_id}, kind: {envelope.kind}, actor_hint: {envelope.actor_hint}"
        )
        ...
        try:
            ...
            if existing_actor_info and existing_actor_info.capability:
                if envelope.kind not in existing_actor_info.capability.accepts:
                    ...
                    logger.error(
                        f"{error_message} Task: {task_id_for_registry}. Accepted: {existing_actor_info.capability.accepts}"
                    )
                    result.status = TaskState.FAILED.value
                    result.metadata["error"] = error_message
                    await self._task_registry.update_task_state(
                        task_id_for_registry, TaskState.FAILED, error_info=error_message
                    )
                    await stream_queue.put(
                        {
                            "error": error_message,
                            "status": result.status,
                            "accepted_kinds": list(existing_actor_info.capability.accepts),
                        }
                    )
                    return
                logger.debug(
                    f"Capability check passed for existing actor {actor_id_for_check}, task: {task_id_for_registry}"
                )
            ...
            if envelope.kind == EnvelopeKind.CONTROL:
                ...
                if not target_control_task_id:
                    logger.error(
                        f"CONTROL envelope {envelope.request_id} must specify a target 'task_id'. This control run ({task_id_for_registry}) will fail."
                    )
                    result.status = TaskState.FAILED.value
                    result.metadata["error"] = "Control envelope lacks target task_id."
                    await self._task_registry.update_task_state(
                        task_id_for_registry, TaskState.FAILED, error_info=result.metadata["error"]
                    )
                ...
                try:
                    ...
                    logger.info(
                        f"Control action '{action.value}' for target_task_id '{target_control_task_id}'. Followup: {bool(followup_data)}. Envelope: {envelope.request_id}"
                    )

                    new_task_state: _t.Optional[TaskState] = None
                    actor_instance: _t.Optional[CallableActor] = None
                    ...
                    if action == ControlAction.PAUSE:
                        ...
                        if actor_instance:
                            try:
                                await actor_instance.pause()
                            except Exception as e:
                                logger.error(
                                    f"Error calling pause on actor {target_task_info.actor_id if target_task_info else 'unknown'}: {e}",
                                    exc_info=True,
                                )
                        else:
                            logger.warning(
                                f"No active actor instance found for {target_task_info.actor_id if target_task_info else 'unknown actor'} (task {target_control_task_id}) to send pause signal."
                            )
                        new_task_state = TaskState.PAUSED
                    elif action == ControlAction.RESUME:
                        logger.info(
                            f"Processing RESUME for task_id '{target_control_task_id}' with followup: {bool(followup_data)}"
                        )
                        if actor_instance:
                            try:
                                await actor_instance.resume(followup_data)
                            except Exception as e:
                                logger.error(
                                    f"Error calling resume on actor {target_task_info.actor_id if target_task_info else 'unknown'}: {e}",
                                    exc_info=True,
                                )
                        else:
                            logger.warning(
                                f"No active actor instance found for {target_task_info.actor_id if target_task_info else 'unknown actor'} (task {target_control_task_id}) to send resume signal."
                            )
                        ...
                        if actor_instance:
                            try:
                                await actor_instance.cancel()
                            except Exception as e:
                                logger.error(
                                    f"Error calling cancel on actor {target_task_info.actor_id if target_task_info else 'unknown'}: {e}",
                                    exc_info=True,
                                )
                        else:
                            logger.warning(
                                f"No active actor instance for {target_task_info.actor_id if target_task_info else 'unknown actor'} (task {target_control_task_id}) to send cancel signal."
                            )
                        new_task_state = TaskState.CANCELED

                    if new_task_state:
                        update_success = await self._task_registry.update_task_state(
                            target_control_task_id, new_task_state
                        )
                        if not update_success:
                            logger.warning(
                                f"Target task_id '{target_control_task_id}' for control action '{action.value}' not found in registry for state update."
                            )
                            result.status = "CONTROL_TARGET_NOT_FOUND"
                            result.metadata["control_error"] = f"Target task {target_control_task_id} not found."
                        else:
                            result.status = f"CONTROL_{action.value.upper()}_PROCESSED"
                    else:
                        result.status = "CONTROL_UNKNOWN_ACTION"

                    result.metadata["control_info"] = (
                        f"Processed control: {action.value} for task {target_control_task_id}"
                    )
                    await stream_queue.put(
                        {"status": result.status, "task_id": target_control_task_id, "action": action.value}
                    )
                except ValidationError as ve:
                    logger.error(f"Invalid ControlPayload for envelope {envelope.request_id}: {ve}", exc_info=True)
                    result.status = TaskState.FAILED.value
                    result.metadata["error"] = f"Invalid ControlPayload: {ve.errors()}"
                    await self._task_registry.update_task_state(
                        task_id_for_registry, TaskState.FAILED, error_info=result.metadata["error"]
                    )
            ...
            if not actors:
                ...
                if not result.metadata.get("error"):
                    result.status = TaskState.FAILED.value
                    result.metadata["error"] = "Actor creation phase failed."
                await self._task_registry.update_task_state(
                    task_id_for_registry, TaskState.FAILED, error_info=result.metadata["error"]
                )
                return
            ...
            if task_info_to_update:
                ...
                logger.error(f"TaskInfo for {task_id_for_registry} not found after actor creation!")
            ...
            if not existing_actor_info:
                ...
                if actor_info_after_creation and actor_info_after_creation.capability:
                    if envelope.kind not in actor_info_after_creation.capability.accepts:
                        error_message = (
                            f"Newly acquired actor {primary_actor_id} does not accept kind '{envelope.kind}'."
                        )
                        logger.error(
                            f"{error_message} Task: {task_id_for_registry}. Accepted: {list(actor_info_after_creation.capability.accepts)}"
                        )
                        result.status = TaskState.FAILED.value
                        result.metadata["error"] = error_message
                        await self._task_registry.update_task_state(
                        ...
                        await stream_queue.put(
                            {
                                "error": error_message,
                                "status": result.status,
                                "accepted_kinds": list(actor_info_after_creation.capability.accepts),
                            }
                        )
                        return
                    logger.debug(
                        f"Capability check passed for new actor {primary_actor_id}, task: {task_id_for_registry}"
                    )
                    ...
                    result.status = TaskState.FAILED.value
                    result.metadata["error"] = f"Info for {primary_actor_id} unavailable post-acquisition."
                    await self._task_registry.update_task_state(
                        task_id_for_registry, TaskState.FAILED, error_info=result.metadata["error"]
                    )
                    await stream_queue.put({"error": result.metadata["error"], "status": result.status})
            ...
            if result.status == TaskState.COMPLETED.value:
                ...
                await self._task_registry.update_task_state(
                    task_id_for_registry,
                    TaskState.FAILED,
                    error_info=result.metadata.get("error", "Unknown error during run"),
                )
    ...
    async def _create_actors(
        self, actors_to_create_spec: list[tuple[str, str]], result: DispatchResult
    ) -> tuple[list[CallableActor], list[str]]:
        actors: list[CallableActor] = []
        created_actor_ids: list[str] = []
    ...
    async def _run_and_collect_outputs(
        self,
        actors: list[CallableActor],
        run_input: ActorRunInput,
        stream_queue: asyncio.Queue[ActorRunOutput | None],
        ...
        for actor_instance in actors:
            runner_group.add_runner(AgentRunner(actor_instance))  # type: ignore
        await runner_group.start_all()
        tasks = [
            asyncio.create_task(self._run_actor_with_semaphore(actor, run_input, stream_queue, result))
            for actor in actors
        ]
        ...
        if exceptions:
            ...
            for e_idx, e_item in enumerate(exceptions):
                result.metadata["errors"].append(f"ActorError{e_idx}: {str(e_item)}")
        else:
    ...
    async def _run_actor(
        ...
        async for output_item in actor.run(run_input):
            processed_output = (
                ...
                else (output_item.dict() if hasattr(output_item, "dict") else output_item)
            )
    ...
    async def _run_actor_with_semaphore(
        self,
        actor: CallableActor,
        run_input: ActorRunInput,
        stream_queue: asyncio.Queue[ActorRunOutput | None],
        ...
        if self._semaphore is None:
            await self._run_actor(actor, run_input, stream_queue, result)
        else:
    ...
    async def cleanup(self) -> None:
        logger.info(
            f"LocalDispatcher cleaning up {len(self._run_tasks)} active run tasks and {len(self._active_runs)} active runs."
        )
        ...
        for runner_id in active_run_keys:
            ...
            if dispatch_result and dispatch_result.task_id:
                task_info = await self._task_registry.get_task(dispatch_result.task_id)
                if task_info and task_info.current_state not in [
                    TaskState.COMPLETED,
                    TaskState.FAILED,
                    TaskState.CANCELED,
                ]:
                    logger.warning(
                        f"Task {dispatch_result.task_id} (runner {runner_id}) was {task_info.current_state.value} during dispatcher cleanup. Marking as FAILED."
                    )
                    await self._task_registry.update_task_state(
                        dispatch_result.task_id,
                        TaskState.FAILED,
                        error_info="Dispatcher cleanup initiated while task was active.",
                    )
