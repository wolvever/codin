...
class CodeAgent(Agent):
    ...
    async def _run_turn(
        ...
        try:
            ...
            if self.debug:
                ...
                print("="*80)
                print(f"📝 Variables being sent to LLM:")
            ...
            tool_results = []
            if tool_calls:
                await self._emit_event("tool_results_start", {
                ...
                await self._emit_event("tool_results", {
            ...
            error_message = self._create_agent_message(f"I encountered an error: {str(e)}", task_id)
            return [error_message], []
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                continue_decision = self._should_continue_execution(
                    iteration, elapsed_time, total_tool_calls
                )
            ...
            error_response = self._create_agent_message(f"I encountered an error: {str(e)}", task_id)
            return AgentRunOutput(
        ...
        if not tool_results:
            ...
            if response_messages:
                ...
                completion_indicators = [
                    ...
                    "successfully created", "successfully executed",
                    "no further", "that's all", "task is complete"
                ]
                
                # Look for continuation indicators
                continuation_indicators = [
                    "let me", "i'll", "next i", "now i", "first i",
                    "tool_calls", "need to", "should", "will"
                ]
    ...
    async def reset_conversation(self) -> None:
        ...
        await self._emit_event("conversation_reset", {
            "new_context_id": self._context_id
        })
                    ...
                    if call.name == "sandbox_write_file" and call.arguments.get("path") == file_path:
                        ...
                        if file_path.endswith(".py"):
                            exec_call = ToolCall(
                                ...
                                name="sandbox_exec",
                                arguments={"cmd": f"python {file_path}"}
                            )
                        else:
                            exec_call = ToolCall(
                                call_id=call.call_id,
                                name="sandbox_exec", 
                                arguments={"cmd": file_path}
                            )
