    ...
    async def run(self, inp: AgentRunInput) -> AgentRunOutput:
        ...
        try:
            ...
            error_message = Message(
                role="assistant",
                parts=[TextPart(text=f"Error creating plan: {str(e)}")]
            )
...
class DAGExecutor(Agent):
    ...
    async def run(self, inp: AgentRunInput) -> AgentRunOutput:
        ...
        try:
            ...
            if result.success:
                response_text = (
                    f"Successfully executed plan: {result.plan.name}\n\n"
            ...
            error_message = Message(
                role="assistant",
                parts=[TextPart(text=f"Error executing plan: {str(e)}")]
            )
                ...
                for attempt in range(self.max_retries):
                    try:
                        ...
                        if attempt == self.max_retries - 1:
                            ...
                            task.status = TaskStatus.FAILED
                            task.error_message = f"Failed after {self.max_retries} attempts: {str(e)}"
