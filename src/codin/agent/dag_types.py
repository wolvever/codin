...
class Task(BaseModel):
    ...
    description: str
    tool: str = ''
    parameters: dict[str, _t.Any] = Field(default_factory=dict)
    requires: list[str] = Field(default_factory=list)  # Task dependencies
    status: TaskStatus = TaskStatus.PENDING
    result: _t.Any = None
    error_message: str | None = None
    error: str | None = None  # For backwards compatibility
    ...
    def update_status(self, status: TaskStatus, result: _t.Any = None, error: str | None = None) -> None:
...
class Plan(BaseModel):
    ...
    def update_status(self) -> None:
        ...
        if all(task.status in (TaskStatus.COMPLETED, TaskStatus.SKIPPED) for task in self.tasks.values()):
        ...
        if any(
            task.status not in (TaskStatus.PENDING, TaskStatus.COMPLETED, TaskStatus.SKIPPED)
            for task in self.tasks.values()
        ):
            self.status = TaskStatus.IN_PROGRESS
            if not self.started_at:
        ...
        if any(task.status in (TaskStatus.COMPLETED, TaskStatus.SKIPPED) for task in self.tasks.values()) and any(
