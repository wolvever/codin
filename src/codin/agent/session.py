...
class Session:
    ...
    def build_state(self) -> State:
        ...
        return State(
            ...
            history=self.messages.copy(),  # Copy to avoid mutations
            task_list=self.task_list.copy(),
            context=self.context.copy(),
            metrics=self.metrics.copy(),
            last_tool_results=self.context.get("last_tool_results")
        )
    ...
    def update_from_state(self, state: State) -> None:
        ...
        if len(state.history) > len(self.messages):
            new_messages = state.history[len(self.messages):]
