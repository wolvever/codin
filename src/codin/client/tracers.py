"""Request tracing utilities for codin agents.

This module provides various tracers for monitoring and debugging HTTP requests
made by codin client components, including logging, metrics collection, and history tracking.
"""
...
class LoggingTracer(RequestTracer):
    ...
    async def on_request_error(self, method: str, url: str, error: Exception, elapsed: float) -> None:
        logger.log(logging.ERROR, f'Failed {method} request to {url} after {elapsed:.3f}s: {error!s}')
...
class RequestHistoryTracer(RequestTracer):
    ...
    async def on_request_start(self, method: str, url: str, headers: dict, data: _t.Any | None = None) -> None:
        record = RequestRecord(method=method, url=url, start_time=time.time())
