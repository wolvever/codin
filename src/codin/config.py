...
def get_default_providers() -> dict[str, ProviderConfig]:
    """Get default provider configurations matching codex."""
    return {
        ...
        'xai': ProviderConfig(
            name='xAI', base_url='https://api.x.ai/v1', env_key='XAI_API_KEY', models=['grok-1', 'grok-beta']
        ),
        'groq': ProviderConfig(
            name='Groq',
...
def load_config(config_file: Path | str | None = None) -> CodinConfig:
    ...
    if 'CODIN_RULE' in os.environ:
        env_overrides['enable_project_docs'] = os.environ['CODIN_RULE'].lower() in ('1', 'true', 'yes')
    ...
    for key, value in config_data.items():
        if hasattr(config, key):
            ...
            if key == 'approval_mode' and isinstance(value, str):
                ...
                # Convert providers dict to ProviderConfig objects
                providers = {}
                for provider_name, provider_data in value.items():
                    if isinstance(provider_data, dict):
                        ...
                        # Already a ProviderConfig object
    ...
    # Set up default providers if not configured
    if not config.providers:
        config.providers = get_default_providers()
