"""Ray-based agent host implementation.

This module provides a host for running agents using Ray for distributed computation.
"""
...
from ..protocol.types import Message, TextPart, AgentRunInput, AgentRunOutput
from ..protocol.types import TaskState, TaskStatus as ProtocolTaskStatus
...
prom_ray_agent_runs = prom.Counter(
    "codin_ray_agent_runs_total", 
    "Number of Ray agent runs",
    ["agent_id", "status"]
)
...
prom_ray_cluster_resources = prom.Gauge(
    "codin_ray_cluster_resources",
    "Available resources in the Ray cluster",
    ["resource_type"]
)
...
def run_agent(agent_state_dict: dict, agent_input_dict: dict) -> dict:
    ...
    try:
        ...
        # Create error output
        from ..protocol.types import TaskState, TaskStatus, Message, TextPart
    ...
    async def send_message_to_agent(
        ...
        with tracer.start_as_current_span(f"ray_send_message_to_agent_{agent_id}") as span:
            ...
            try:
                ...
                error_message = Message(
                    role="assistant",
                    parts=[TextPart(text=f"Ray execution error: {str(e)}")]
                )
...
async def create_ray_agent_host(
    ...
    with tracer.start_as_current_span("create_ray_agent_host") as span:
        ...
        for spec in agent_specs:
            ...
            # Create a basic agent (customize as needed)
            agent = Agent(
                id=agent_id,
                name=agent_name,
