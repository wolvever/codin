...
class OpenAIEmbedding(BaseEmbedding):
    ...
    async def __init__(self, model: str, config: ModelConfig | None = None):  # Changed to async
        ...
        if not api_key:
            raise ValueError("API key not found. Set in ModelConfig or OPENAI_API_KEY environment variable.")

        # Base URL: config > env (specific OPENAI_API_BASE) > default
        base_url = self.config.base_url or os.getenv("OPENAI_API_BASE") or self.DEFAULT_BASE_URL
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to decode JSON response from OpenAI Embedding for model {self.model}: {e}. Response text: {response.text}"
            )
        ...
        try:
            ...
            if not isinstance(embedding_data, list):
                ...
                raise ModelResponseParsingError(
                    f"Invalid response format from model {self.model}: 'data' field missing or not a list."
                )
            ...
            for item in sorted(embedding_data, key=lambda x: x.get("index", -1)):
                if isinstance(item, dict) and "embedding" in item:
                    ...
                    logger.warning(
                        f"Skipping item in embedding response for model {self.model} due to missing 'embedding' key or wrong type: {item}"
                    )
            ...
            logger.error(
                f"OpenAI Embedding response parsing error for model {self.model}: {type(e_parse).__name__} - {e_parse}."
            )
            logger.debug(f"Problematic data for model {self.model}: {response_data}")
            raise ModelResponseParsingError(
                f"Invalid response format from model {self.model}, parsing error: {e_parse}"
            ) from e_parse
        except Exception as e_other:  # Catch any other unexpected error during processing
            logger.error(
                f"Unexpected error processing OpenAI Embedding response for model {self.model}: {e_other}.",
                exc_info=True,
            )
            logger.debug(f"Problematic data for model {self.model}: {response_data}")
            raise ContentExtractionError(
                f"Unexpected error processing embeddings for model {self.model}: {e_other}"
            ) from e_other
