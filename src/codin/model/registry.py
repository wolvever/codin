...
class ModelRegistry:
    ...
    def register(model_cls: type[BaseModel]) -> type[BaseModel]:
        ...
        for pattern in model_cls.supported_models():
            if pattern in _model_registry:
                ...
                logger.warning(
                    f"Replacing model for pattern '{pattern}': {existing_cls.__name__} → {model_cls.__name__}"
                )
    ...
    def list_supported_models(model_type: ModelType | None = None) -> list[str]:
        ...
        return [pattern for pattern, (registry_type, _) in _model_registry.items() if registry_type == model_type]
