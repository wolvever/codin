...
class MCPSessionManager(abc.ABC):
    ...
    def create(
        ...
        **kwargs:
        ...
        elif isinstance(connection_params, StdioServerParams):
            if not HAS_MCP_CLIENT:
                raise ImportError(
                    "To use stdio connections, install the 'mcp' package."
                )
            return StdioSessionManager(connection_params, **kwargs)  
        elif isinstance(connection_params, SseServerParams):
            if not HAS_MCP_CLIENT:
                raise ImportError(
                    "To use SSE connections, install the 'mcp' package."
                )
            return SseSessionManager(connection_params, **kwargs)
        else:
            raise ValueError(
                f"Unsupported connection parameters type: {type(connection_params)}"
            )
